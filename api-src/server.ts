...
const authRouter = t.router({
  signup: publicProcedure.input(z.object({ name: z.string().min(2), email: z.string().email(), password: z.string().min(8) })).mutation(async ({ input, ctx }) => {
...
const businessRouter = t.router({
  create: protectedProcedure.input(z.object({ name: z.string().min(2), slug: z.string().min(2), businessType: z.string().optional(), phone: z.string().optional(), email: z.string().optional(), tagline: z.string().optional(), keywords: z.array(z.string()).optional() })).mutation(async ({ input, ctx }) => {
    ...
    if (existing[0]) throw new Error("That URL slug is taken");
    ...
    const bizId = result[0].id;
    await db.insert(platforms).values(DEFAULT_PLATFORMS.map(p => ({ ...p, businessId: bizId })));
    const biz = await db.select().from(businesses).where(eq(businesses.id, bizId)).limit(1);
    return biz[0];
  }),

  mine: protectedProcedure.query(async ({ ctx }) => {
    return db.select().from(businesses).where(eq(businesses.ownerId, ctx.user.id)).orderBy(desc(businesses.createdAt));
  }),
  ...
  platforms: protectedProcedure.input(z.object({ businessId: z.number() })).query(async ({ input }) => {
    return db.select().from(platforms).where(eq(platforms.businessId, input.businessId)).orderBy(platforms.sortOrder);
  }),
  ...
  staff: protectedProcedure.input(z.object({ businessId: z.number() })).query(async ({ input }) => {
    return db.select().from(staff).where(eq(staff.businessId, input.businessId)).orderBy(desc(staff.reviews));
  }),
  ...
  leads: protectedProcedure.input(z.object({ businessId: z.number() })).query(async ({ input }) => {
    return db.select().from(leads).where(eq(leads.businessId, input.businessId)).orderBy(desc(leads.createdAt));
  }),
...
const adminRouter = t.router({
  allBusinesses: protectedProcedure.query(async ({ ctx }) => {
  ...
  allUsers: protectedProcedure.query(async ({ ctx }) => {
    if (ctx.user.role !== "admin") throw new TRPCError({ code: "FORBIDDEN" });
    return db.select({ id: users.id, email: users.email, name: users.name, role: users.role, createdAt: users.createdAt }).from(users).orderBy(desc(users.createdAt));
  }),
});
